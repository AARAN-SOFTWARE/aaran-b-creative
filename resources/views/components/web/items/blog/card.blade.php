...
@foreach($firstPost as $first)
    <div class="bg-white rounded-2xl">
        <div class="relative">
            <img src="{{ \Illuminate\Support\Facades\Storage::url('/images/'.$first->image) }}" alt=""
                 class="w-full h-[24rem] rounded-t-2xl">
            <div
                class="max-w-max bg-orange-600 text-xs text-white px-2 py-1 rounded absolute top-6 left-6 shadow-sm shadow-gray-900">
                {{ \Aaran\Blog\Models\BlogPost::tagName($first->blogtag_id) ?: 'posts'}}
            </div>
        </div>

        <div class="pt-5 space-y-5">
            <div class="px-5 font-semibold text-[10px] text-orange-700">{{ $first->created_at->diffForHumans() }}</div>
            <div
                class="px-5 uppercase font-merri text-2xl tracking-wider">{{\Illuminate\Support\Str::words($first->vname,10)}}</div>
            ...
            <div class="px-5 text-xs border-t py-5 flex justify-between items-center">
                <span class="text-gray-500">Post By - <span
                        class="text-orange-700 font-normal">{{$first->user->name,10}}</span></span>
                ...
                <span>
                        <button wire:click="edit({{ $first->id }})" class="rounded-md ">

                            <x-icons.icon :icon="'pencil'"
                                          class="block w-auto h-4 text-blue-700 hover:scale-110"/>
                        </button>

                        <button wire:click="getDelete({{ $first->id }})" class="rounded-md ">

                        <x-icons.icon :icon="'trash'"
                                      class="block w-auto h-4 text-red-700 hover:scale-110"/>
                        </button>
                    </span>
            </div>
        </div>
...
@foreach($list->skip(1) as $row)
    <div class="bg-white rounded-2xl">
        ...
        <div class="pt-5 space-y-5 ">
            <div
                class="px-5 font-semibold text-[10px] text-orange-700">{{ $row->created_at->diffForHumans() }}</div>
            <div
                class="px-5 uppercase font-merri text-2xl tracking-wider">{{\Illuminate\Support\Str::words($row->vname,10)}}</div>
            <div class="px-5 text-xs text-gray-400 leading-loose tracking-wider">
            ...
            <div class="px-5 text-xs border-t py-5 flex justify-between items-center">
                        <span class="text-gray-500">Post By - <span
                                class="text-orange-700 font-normal">{{$row->user->name,10}}</span></span>
                <span class="inline-flex items-center space-x-4">
                            <span>
                                <svg width="" height="" viewBox="0 0 90 90" fill="none"
                                     xmlns="http://www.w3.org/2000/svg" class=" w-4 h-4 ">
